...
const loadData = async () => {
    try {
        const data = await uraianSdmController.getAll();
...
const showDrawer = async (data) => {
    try {
        ...
        // console.log(data);
        if (data != null) {
            const response = await uraianSdmController.getByID(data.id);
...
const submitData = async () => {
    ...
    if (!formData.value.name) {
        ...
        if (statusForm.value == 'add') {
            const response = await uraianSdmController.addPost(formData.value);
            ...
            const response = await uraianSdmController.updatePost(formData.value.id, formData.value);
            // const load = response.data;
...
<template>
    <div class="flex flex-col w-full gap-8">
        <div class="flex gap-2 items-center justify-between w-full font-bold">
            <span class="text-3xl">Uraian SDM</span>
        ...
        <Drawer v-model:visible="drawerCond" position="right" class="!w-full md:!w-[30rem]">
            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <InputText v-model="formData.name" placeholder="Input Nama Uraian" />
                </div>
        ...
        <Card>
            <template #title>
                <div class="flex gap-2 items-center mb-5">
                    <span class="text-xl font-bold w-full">List</span>
                    <InputGroup>
                ...
                <ScrollPanel style="width: 100%; height: 32vw">
                    <div class="flex flex-col gap-6">
                        <div class="w-full rounded-xl p-3 border flex gap-3 items-center" v-for="(item, index) in filteredList" :key="index">
                            <i class="pi pi-bookmark-fill p-3 rounded-xl bg-gray-200 text-black" style="font-size: 1.2vw"></i>
