...
const loadUraian = async () => {
    const response = await uraianSdmController.getAll();
...
const formCondition2 = async () => {
    const tanggal = moment(formData.value.tanggal).format('YYYY-MM-DD');
    const tabel = listTable.value;
    if (tabel.length > 0) {
        const data = tabel.find((item) => item.tanggal == tanggal);
        if (data != null) {
            statusForm.value = 'edit';
            formData.value.id = data.id;
            formData.value.uraian_id = data.uraian_id;
            formData.value.nilai = data.nilai;

            try {
                const response = await dataSdmController.getByID(data.id);
                if (response && response.history) {
                    logFile.value = response.history.map((historyItem) => ({
                        action: historyItem.action,
                        user_name: historyItem.user_name,
                        date: moment(historyItem.created_at).format('DD MMM YYYY - HH:mm:ss'),
                        changes: []
                    }));
                } else {
                    logFile.value = [];
...
const formCondition = async () => {
    if (!formData.value.tanggal) return; // Cek apakah tanggal tidak null

    // Pastikan allData sudah memiliki nilai
    if (!allData.value || allData.value.length === 0) {
        console.warn('allData belum memiliki data!');
        return;
    }

    const tgl = moment(formData.value.tanggal, 'YYYY-MM-DD', true).format('YYYY-MM-DD');
    const data = allData.value.find((item) => {
        if (!item.tanggal) return false; // Hindari error jika item.tanggal null/undefined
        return moment(item.tanggal, 'YYYY-MM-DD', true).format('YYYY-MM-DD') === tgl;
    });
...
const submitData = async () => {
    if (!formData.value.uraian_id || !formData.value.tanggal || formData.value.nilai == null) {
        ...
        if (statusForm.value == 'add') {
            const response = await dataSdmController.addPost(formData.value);
            ...
            const response = await dataSdmController.updatePost(formData.value.id, formData.value);
            // const load = response.data;
            console.log(response);
            if (response.status == true) {
...
<template>
    <div class="flex flex-col w-full gap-8">
        <div class="flex gap-2 items-center justify-between w-full font-bold">
            <span class="text-3xl">Data SDM</span>
        ...
        <Drawer v-model:visible="drawerCond" position="right" class="!w-full md:!w-[30rem]">
            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-1 w-full">
                    ...
                    <DatePicker v-model="formData.tanggal" :maxDate="maxDate" dateFormat="yy-mm-dd" showIcon placeholder="Please input Date" @update:modelValue="formCondition2" />
                </div>
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <InputNumber v-model="formData.nilai" inputId="minmaxfraction" placeholder="1,000,000" :minFractionDigits="0" :maxFractionDigits="2" fluid />
                </div>
                ...
                <div class="flex flex-col gap-3">
                    <ScrollPanel style="width: 100%; height: 35vw">
                        <div class="grid grid-cols-3 gap-3">
                            <Panel v-for="(item, index) in listTable" :key="index" :collapsed="true">
                                <template #header>
