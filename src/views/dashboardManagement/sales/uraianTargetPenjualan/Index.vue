...
const loadData = async () => {
    try {
        const data = await uraianPenjualanController.getAll();
...
const showDrawer = async (data) => {
    try {
        ...
        // console.log(data);
        if (data != null) {
            const response = await uraianPenjualanController.getByID(data.id);
...
const submitData = async () => {
    ...
    if (!formData.value.nama) {
        ...
        if (statusForm.value == 'add') {
            const response = await uraianPenjualanController.addPost(formData.value);
            ...
            console.log(formData.value);
            const response = await uraianPenjualanController.updatePost(formData.value.id, formData.value);
            // const load = response.data;
...
<template>
    <div class="flex flex-col w-full gap-8">
        <div class="flex gap-2 items-center justify-between w-full font-bold">
            <span class="text-3xl">Uraian Target Penjualan</span>
            <button @click="showDrawer(null)" class="px-4 py-2 font-bold items-center shadow-lg hover:shadow-none transition-all duration-300 bg-emerald-500 hover:bg-emerald-700 text-white rounded-full flex gap-2">
                ...
                <span>Tambah Jenis Target</span>
            </button>
        </div>
        <Drawer v-model:visible="drawerCond" position="right" class="!w-full md:!w-[30rem]">
            <template #header>
                <span class="text-[1vw] font-bold">Form Jenis Target</span>
            </template>
            <template #footer>
            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <InputText v-model="formData.nama" placeholder="Input Nama Target" />
                </div>
        ...
        <Card>
            <template #title>
                <div class="flex gap-2 items-center mb-5">
                    <span class="text-xl font-bold w-full">List Target</span>
                    <InputGroup>
            ...
            <template #content>
                <ScrollPanel style="width: 100%; height: 32vw">
                    <div class="flex flex-col gap-6">
                        <div class="w-full rounded-xl p-3 border flex gap-3 items-center" v-for="(item, index) in filteredList" :key="index">
                            <i class="pi pi-bullseye p-3 rounded-xl bg-gray-200 text-black" style="font-size: 1.2vw"></i>
