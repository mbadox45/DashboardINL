...
const loadJenis = async () => {
    try {
        const response = await productStorageScmController.getByJenis('warehouse');
        const list = [];
...
const loadProduk = async () => {
    ...
    const produk = response.filter((item) => item.jenis.toLowerCase().includes('ritel'));
...
const postData = async (cond) => {
    if (cond == 'back') {
        router.push('/scm/stock/retail');
        // window.location.replace(`${URL_WEB}scm/stock/retail`);
    } else {
        loadings.value = true;
        const response = await retailStockScmController.postData(formData.value);
