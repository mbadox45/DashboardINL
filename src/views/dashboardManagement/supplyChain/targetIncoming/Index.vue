...
const loadData = async () => {
    try {
        const data = await targetIncomingScmController.getAll();
        ...
        if (data != null) {
            ...
            for (let i = 0; i < history.length; i++) {
                ...
                if (history[i].changes.length == 0) {
                    ...
                    from = history[i].changes.qty.old;
                    to = history[i].changes.qty.new;
                }
                list.push({
...
const submitData = async () => {
    if (!formData.value.qty || !formData.value.tanggal) {
...
const filteredList = computed(() => {
    if (!search.value) {
        ...
        return listTable.value.filter((item) => item.tanggal.includes(search.value));
    }
});
</script>
...
<template>
    <div class="flex flex-col w-full gap-8">
        <div class="flex gap-2 items-center justify-between w-full font-bold">
            <span class="text-5xl">Target Incoming (RKAP)</span>
        ...
        <Card>
            <template #title>
                <div class="flex gap-2 items-center mb-5">
                    ...
                    <InputGroup>
                        <DatePicker v-model="search" dateFormat="yy-mm-dd" placeholder="Search by Date" />
                        <InputGroupAddon>
                        ...
                        <div class="p-3 flex w-full gap-3 items-center border rounded-lg" v-for="(item, index) in filteredList" :key="index">
                            <div class="flex w-full gap-8 items-center">
                                ...
                                <div class="flex flex-col w-full">
                                    <span class="w-full text-[0.9vw]">{{ moment(item.tanggal).format('DD MMMM YYYY') }}</span>
                                    <span class="w-full text-[1vw] font-bold">{{ formatCurrency(Number(item.qty).toFixed(2)) }}</span>
                                </div>
