...
const loadData = async () => {
    try {
        const data = await pmgMasterController.getAll();
...
const filteredList = computed(() => {
    return listTable.value.filter((item) => item.nama.toLowerCase().includes(search.value.toLowerCase()) || item.lokasi.toLowerCase().includes(search.value.toLowerCase()));
});
</script>
...
<template>
    <div class="flex flex-col w-full gap-8">
        <div class="flex gap-2 items-center justify-between w-full font-bold">
            <span class="text-5xl">Master PMG</span>
