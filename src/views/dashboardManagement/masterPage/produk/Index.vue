...
const loadData = async () => {
    try {
        const data = await productMasterController.getAll();
...
const showDrawer = async (data) => {
    try {
        ...
        messages.value = [];
        if (data != null) {
            const response = await productMasterController.getByID(data.id);
            ...
            formData.value.konversi_pallet = Number(data.konversi_pallet);
            formData.value.konversi_pouch = Number(data.konversi_pouch);
            formData.value.konversi_ton = Number(data.konversi_ton);
            ...
            formData.value.konversi_pallet = null;
            formData.value.konversi_pouch = null;
            formData.value.konversi_ton = null;
...
const valueCondition = () => {
    if (formData.value.jenis == 'bulk') {
        formData.value.konversi_pallet = 1;
        formData.value.konversi_pouch = 1;
        formData.value.konversi_ton = 1;
    } else {
        formData.value.konversi_pallet = null;
        formData.value.konversi_pouch = null;
        formData.value.konversi_ton = null;
...
<template>
    <div class="flex flex-col w-full gap-8">
        <div class="flex gap-2 items-center justify-between w-full font-bold">
            <span class="text-3xl">Master Product</span>
        ...
        <Drawer v-model:visible="drawerCond" position="right" class="!w-full md:!w-[30rem]">
            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <Select v-model="formData.jenis" :options="jenisProduk" optionLabel="name" optionValue="id" placeholder="Select a Type" class="w-full" @change="valueCondition" />
                </div>
                <div class="flex flex-col gap-1">
                    <label for="konversi_pallet">Konversi Pallet <small class="text-red-500 font-bold">*</small></label>
                    <InputNumber :disabled="formData.jenis == 'bulk' ? true : false" v-model="formData.konversi_pallet" inputId="minmaxfraction" placeholder="1,000,000" :minFractionDigits="0" :maxFractionDigits="2" fluid />
                </div>
                <div class="flex flex-col gap-1">
                    <label for="konversi_pouch">Konversi Pouch <small class="text-red-500 font-bold">*</small></label>
                    <InputNumber :disabled="formData.jenis == 'bulk' ? true : false" v-model="formData.konversi_pouch" inputId="minmaxfraction" placeholder="1,000,000" :minFractionDigits="0" :maxFractionDigits="2" fluid />
                </div>
                <div class="flex flex-col gap-1">
        ...
        <Card>
            ...
            <template #content>
                <DataTable :value="listTable" showGridlines paginator :rows="10" dataKey="period">
                    ...
                    <Column field="konversi_ton" sortable style="width: 25%; font-size: 0.7vw">
                        <template #header>
                            <div class="flex w-full justify-center">
                                <span>Konversi Ton</span>
                            </div>
                        </template>
                        <template #body="{ data }">
                    ...
                    <Column field="konversi_pallet" sortable style="width: 25%; font-size: 0.7vw">
                        <template #header>
                            <div class="flex w-full justify-center">
                                <span>Konversi Pallet</span>
                            </div>
                        </template>
                        <template #body="{ data }">
                            <div class="flex w-full justify-end">
                                <span class="capitalize">{{ formatCurrency(Number(data.konversi_pallet).toFixed(2)) }}</span>
                            </div>
                        </template>
                    </Column>
                    <Column field="konversi_pouch" sortable style="width: 25%; font-size: 0.7vw">
                        <template #header>
                            <div class="flex w-full justify-center">
                                <span>Konversi Pouch</span>
                            </div>
                        </template>
                        <template #body="{ data }">
                            <div class="flex w-full justify-end">
                                <span class="capitalize">{{ formatCurrency(Number(data.konversi_pouch).toFixed(2)) }}</span>
                            </div>
                        </template>
                    </Column>
