...
const loadData = async () => {
    try {
        const data = await lokasiMasterController.getAll();
...
const showDrawer = async (data) => {
    try {
        ...
        console.log(data);
        if (data != null) {
            const response = await lokasiMasterController.getByID(data.id);
...
const submitData = async () => {
    if (formData.value.name != null) {
        if (statusForm.value == 'add') {
            ...
            console.log(formData.value);
            const response = await lokasiMasterController.updatePost(formData.value.id, formData.value);
            // const load = response.data;
...
<template>
    <div class="flex flex-col w-full gap-8">
        <div class="flex gap-2 items-center justify-between w-full font-bold">
            <span class="text-5xl">Master Lokasi</span>
            ...
            <template #content>
                <ScrollPanel style="width: 100%; height: 32vw">
                    <div class="flex flex-col gap-6">
                        <div class="w-full border rounded-xl px-5 py-3 flex gap-3 items-center" v-for="(item, index) in filteredList" :key="index">
                            <i class="pi pi-paperclip p-3 rounded-xl bg-gray-200" style="font-size: 2vw"></i>
