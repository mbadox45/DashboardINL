...
const loadData = async () => {
    try {
        const data = await customerPartnerMasterController.getAll();
...
const showDrawer = async (data) => {
    try {
        ...
        console.log(data);
        if (data != null) {
            const response = await customerPartnerMasterController.getByID(data.id);
...
const submitData = async () => {
    if (!formData.value.name || !formData.value.email || !formData.value.kontak || !formData.value.kota || !formData.value.provinsi || !formData.value.negara || !formData.value.alamat) {
        ...
        if (statusForm.value == 'add') {
            const response = await customerPartnerMasterController.addPost(formData.value);
            ...
            const response = await customerPartnerMasterController.updatePost(formData.value.id, formData.value);
            // const load = response.data;
...
<template>
    <div class="flex flex-col w-full gap-8">
        <div class="flex gap-2 items-center justify-between w-full font-bold">
            <span class="text-5xl">Master Customer</span>
        ...
        <Card>
            ...
            <template #content>
                <DataTable v-model:filters="search" :value="listTable" showGridlines dataKey="period" :globalFilterFields="['name', 'email', 'kontak', 'negara', 'provinsi', 'kota', 'alamat']">
