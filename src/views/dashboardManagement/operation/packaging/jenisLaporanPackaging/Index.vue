<script setup>
import { condFunction, conditions, kategori } from '@/controller/getApiFromThisApp/dummy/variableDummy';
...
const loadData = async () => {
    try {
        const data = await jenisLaporanPackagingController.getAll();
...
const showDrawer = async (data) => {
    try {
        ...
        messages.value = [];
        if (data != null) {
            const response = await jenisLaporanPackagingController.getByID(data.id);
            ...
            const item = data.item_packaging;
            const listItem = [];
            for (let i = 0; i < item.length; i++) {
                let name;
                if (item[i].kategori == 'produk_hasil') {
                    const produk = listProduct.value.find((items) => items.name == item[i].name);
                    name = produk.id;
                } else {
                    name = item[i].name;
                }
...
const loadProduct = async () => {
    ...
    const produk = response.filter((item) => item.jenis == 'ritel');
    listProduct.value = produk;
};

const testChange = async (val) => {
    checkKategori.value = val;
};
...
const submitData = async () => {
    if (!formData.value.name || !formData.value.condition_olah) {
        ...
        const response = await jenisLaporanPackagingController.postData(formData.value, statusForm.value);
...
<template>
    <div class="flex flex-col w-full gap-8">
        <div class="flex gap-2 items-center justify-between w-full font-bold">
            <span class="text-3xl">Jenis Laporan Packaging</span>
        ...
        <Drawer v-model:visible="drawerCond" position="right" class="!w-full md:!w-[30rem]">
            <template #header>
                <div class="flex flex-col gap-3">
                    <span class="text-[1vw] font-bold">Form Jenis Packaging</span>
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <div class="flex flex-col gap-2 p-4 border rounded-lg" v-for="(items, index) in formData.items" :key="index">
                        <div class="flex flex-col gap-1">
                            ...
                            <InputText v-if="items.kategori != 'produk_hasil'" type="text" v-model="items.name" placeholder="Input Nama Item" size="small" />
                            <Select v-else v-model="items.name" :options="listProduct" optionLabel="name" filter optionValue="id" size="small" placeholder="Select a Item" class="w-full" />
                        </div>
                        <div class="flex flex-col gap-1">
                            <label for="nama">Kategori Item <small class="text-red-500 font-bold">*</small></label>
                            <Select v-model="items.kategori" :options="listKategori" optionLabel="name" optionValue="id" size="small" placeholder="Pilih Jenis Item" class="w-full" />
                        </div>
                ...
                <ScrollPanel style="width: 100%; height: 35vw">
                    <div class="flex flex-col w-full gap-3">
                        <Panel v-for="(item, index) in filteredList" :key="index" toggleable :collapsed="true" class="w-full">
                            ...
                            <DataTable :value="item.item_packaging" showGridlines class="mt-4">
