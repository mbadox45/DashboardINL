...
const loadData = async () => {
    try {
        const data = await uraianPackagingController.getAll();
...
const showDrawer = async (data) => {
    try {
        ...
        console.log(data);
        if (data != null) {
            const response = await uraianPackagingController.getByID(data.id);
...
const submitData = async () => {
    ...
    if (!formData.value.nama) {
        ...
        if (statusForm.value == 'add') {
            const response = await uraianPackagingController.addPost(formData.value);
            ...
            console.log(formData.value);
            const response = await uraianPackagingController.updatePost(formData.value.id, formData.value);
            // const load = response.data;
