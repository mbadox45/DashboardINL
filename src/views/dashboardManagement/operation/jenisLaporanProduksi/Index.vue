...
const loadData = async () => {
    try {
        const data = await jenisLaporanProduksiController.getAll();
...
const showDrawer = async (data) => {
    try {
        ...
        messages.value = [];
        if (data != null) {
            const response = await pmgMasterController.getByID(data.id);
            ...
            for (let i = 0; i < history.length; i++) {
                ...
                list.push({
                    ...
                    changesLokasi:
                        fromLokasi == null && toLokasi == null
                            ? []
                            : [
                                  { icon: 'pi pi-arrow-up text-red-500', name: fromLokasi },
                                  { icon: 'pi pi-arrow-down text-green-500', name: toLokasi }
                              ],
                    changesNama:
...
<template>
    <div class="flex flex-col w-full gap-8">
        <div class="flex gap-2 items-center justify-between w-full font-bold">
            <span class="text-4xl">Master Jenis Produksi</span>
        ...
        <Drawer v-model:visible="drawerCond" position="right" class="!w-full md:!w-[30rem]">
            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <InputText type="text" v-model="formData.nama" placeholder="Please input Name" />
                </div>
                <div class="flex flex-col gap-1">
                    <label for="nama">Lokasi <small class="text-red-500 font-bold">*</small></label>
                    <InputText type="text" v-model="formData.lokasi" placeholder="Please input Location" />
                </div>
        ...
        <Card>
            ...
            <template #content>
                <ScrollPanel style="width: 100%; height: 33vw">
                    <div class="flex flex-col gap-3">
                        <Panel v-for="(item, index) in filteredList" :key="index" toggleable :collapsed="true">
                            <template #header>
                                <div class="flex gap-5 items-center">
                                    <button @click="showDrawer(item)" class="p-3 border rounded-full flex justify-center items-center hover:bg-amber-300 shadow-md transition-all duration-300">
                                        <i class="pi pi-pencil" style="font-size: 0.6vw"></i>
                                    </button>
                                    <span class="text-[1vw] font-bold italic font-sans">{{ item.name }}</span>
                                </div>
                            </template>
                            <div class="px-3 grid grid-cols-4 gap-4 mt-3">
                                <div class="py-2 px-4 border rounded-xl w-full flex" v-for="(items, ind) in item.item_produksi" :key="ind">
                                    <div class="flex items-center">
                                        <span class="text-[1vw] font-bold italic font-sans">{{ items.name }}</span>
                                    </div>
                                </div>
                            </div>
                        </Panel>
