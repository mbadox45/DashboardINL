...
const loadData = async () => {
    try {
        const data = await jenisLaporanProduksiController.loadTable();
...
const showDrawer = async (data) => {
    try {
        ...
        messages.value = [];
        if (data != null) {
            const response = await jenisLaporanProduksiController.getByID(data.id);
...
const submitData = async () => {
    if (!formData.value.name || !formData.value.condition_olah) {
        ...
        const response = await jenisLaporanProduksiController.postData(formData.value, statusForm.value);
...
<template>
    <div class="flex flex-col w-full gap-8">
        <div class="flex gap-2 items-center justify-between w-full font-bold">
            <span class="text-3xl">Jenis Laporan Produksi</span>
        ...
        <Drawer v-model:visible="drawerCond" position="right" class="!w-full md:!w-[30rem]">
            <template #header>
                <div class="flex flex-col gap-3">
                    <span class="text-[1vw] font-bold">Form Jenis</span>
