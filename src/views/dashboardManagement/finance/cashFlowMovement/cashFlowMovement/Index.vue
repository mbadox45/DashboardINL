...
const exportToExcel = async () => {
    ...
    const response = await cashFlowMovementController.loadToExportTable(form);
    ...
    saveAs(data, `Data-Cash-Flow-Movement-Cash-Balance-${moment().format('YYYY-MM-DD-HHmmss')}.xlsx`);
};
...
const submitData = async () => {
    if (formData.value.tanggal != null && formData.value.kategori_id != null && formData.value.value != null) {
        ...
        if (statusForm.value == 'add') {
            ...
            const response = await cashFlowMovementController.updatePost(formData.value.id, formData.value);
