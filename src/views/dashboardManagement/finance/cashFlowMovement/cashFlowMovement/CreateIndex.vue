...
const loadJenis = async () => {
    try {
        const response = await kategoriCashFlowMovementController.getAll();
        const list = [];
...
const postData = async (cond) => {
    if (cond == 'back') {
        ...
        const response = await cashFlowMovementController.postData(formData.value);
...
<template>
    <div class="flex flex-col w-full gap-8">
        <div class="flex gap-2 items-center justify-between w-full font-bold">
            <span class="text-3xl">Form Data (Cash Flow Movement)</span>
