...
const loadData = async () => {
    try {
        const data = await payStatusCashFlowScheduleController.getAll();
...
const showDrawer = async (data) => {
    try {
        ...
        messages.value = [];
        if (data != null) {
            const response = await payStatusCashFlowScheduleController.getByID(data.id);
...
const submitData = async () => {
    if (!formData.value.name) {
        ...
        if (statusForm.value == 'add') {
            const response = await payStatusCashFlowScheduleController.addPost(formData.value);
            ...
            const response = await payStatusCashFlowScheduleController.updatePost(formData.value.id, formData.value);
            // const load = response.data;
            console.log(response);
            if (response.status == true) {
            ...
            <template #content>
                <div class="flex flex-col gap-3">
                    <div class="flex gap-3 items-center p-3 shadow rounded-lg" v-for="(item, index) in filteredList" :key="index">
                        <i class="pi px-3 py-3 rounded-lg" :class="item.state == 0 ? 'pi-spinner-dotted bg-amber-300' : 'pi-check-circle bg-emerald-300'"></i>
                        <div class="flex flex-col items-start gap-1 w-full">
                            <span class="w-full text-[1.3vw] font-bold">{{ item.name }}</span>
                            <span class="px-2 py-1 w-auto rounded-full bg-gray-200 text-[0.9vw]">Remark: {{ item.remark }}</span>
                        </div>
