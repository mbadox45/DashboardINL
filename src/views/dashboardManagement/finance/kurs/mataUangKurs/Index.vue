...
const showDrawer = async (data) => {
    try {
        ...
        messages.value = [];
        if (data != null) {
            const response = await mataUangKursController.getByID(data.id);
            ...
            formData.value.symbol = data.symbol;
...
const submitData = async () => {
    if (!formData.value.name || !formData.value.symbol || !formData.value.remark) {
        messages.value = [{ severity: 'warn', content: 'Harap di data lengkapi !', id: count.value++, icon: 'pi-exclamation-triangle' }];
    } else {
        if (statusForm.value == 'add') {
            const response = await mataUangKursController.addPost(formData.value);
            ...
            const response = await mataUangKursController.updatePost(formData.value.id, formData.value);
            // const load = response.data;
