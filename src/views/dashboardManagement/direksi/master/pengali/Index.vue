...
const loadData = async () => {
    try {
        const data = await pengaliSicalRspController.getAll();
...
const showDrawer = async (data) => {
    try {
        ...
        messages.value = [];
        if (data != null) {
            const response = await pengaliSicalRspController.getByID(data.id);
...
const submitData = async () => {
    if (formData.value.name != null && formData.value.value != null) {
        if (statusForm.value == 'add') {
            const response = await pengaliSicalRspController.addPost(formData.value);
            ...
            const response = await pengaliSicalRspController.updatePost(formData.value.id, formData.value);
            if (response.status == true) {
...
<template>
    <div class="flex flex-col w-full gap-8">
        <div class="flex gap-2 items-center justify-between w-full font-bold">
            <span class="text-3xl">Master Pengali</span>
        ...
        <Drawer v-model:visible="drawerCond" position="right" class="!w-full md:!w-[30rem]">
            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-1">
                    <label for="name">Nilai Pengali <small class="text-red-500 font-bold">*</small></label>
