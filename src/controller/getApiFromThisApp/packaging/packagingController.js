import packagingAPI from '@/api/thisAPI/packaging/packagingAPI';
import { msg_error, msg_success, msg_warning } from '@/controller/getApiFromThisApp/dummy/notificationDummy';

export default new (class packagingController {
    addPost = async (form) => {
        try {
            const response = await packagingAPI.addPost(form);
    ...
    updatePost = async (id, form) => {
        try {
            const response = await packagingAPI.updatePost(id, form);
            const load = response.data;
            console.log(load);
            if (load.success == true) {
                return msg_success;
